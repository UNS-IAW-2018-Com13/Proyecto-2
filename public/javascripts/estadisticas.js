function drawHBarChart(target, data, params) {
                ...
                var p = {
                    ...
                    max: 0,
                    xInc: 3,
                    labelWidth: 1, // bootstrap grid col width (i.e. 3/12)
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (firstTime || $bar.length === 0) {
                        var $newbar = $('<div class="row" data-item-id="' + d.name +
                                '" style="margin-top:3px;"><div class="col-sm-' + p.labelWidth + ' chart-name">' + 
                                
                                '<a href="#" data-toggle="tooltip" title="'+name+'"><img src="/images/portraits/portrait'+
                                name + '.jpg" width="42" heigth=42">' +
                                '</a>'+
                                '</div><div class="col-sm-' + (12 - p.labelWidth) + '"><div class="progress-bar"' +
                                ' data-percentage="' + pct + '" style="background-color:' + color +
                                ';width:' + (firstTime && p.initAnimation ? 0 : pct) + '%;">'+d.count+'</div></div></div>');
                        ...
                        if ($bar.attr('data-item-id') !== d.name) {
                            $bar.attr('data-item-id', d.name).find('.chart-name').attr('data-anim-name', name).fadeOut(function () {
